    ...
    def forward_classifier(self, data_batch, train):
        ...
        if train:
            ...
            with torch.no_grad():
                molecular_vectors = self.gnn(inputs)
                predicted_scores = self.mlp(molecular_vectors)
            predicted_scores = torch.sigmoid(predicted_scores)  # Apply sigmoid activation
...
class Tester(object):
    ...
    def test_classifier(self, dataset):
        ...
        tempP = np.concatenate(P)
        y_pred = tempP >= 0.5
        # AUC = roc_auc_score(np.concatenate(C), np.concatenate(P))
        f1 = f1_score(np.concatenate(C), y_pred)
        return f1
...
if __name__ == "__main__":
    ...
    if task == 'classification':
        result = 'Epoch\tTime(sec)\tLoss_train\tAUC_dev\tAUC_test\tF1_score'
    ...
    for epoch in range(iteration):
        ...
        if task == 'classification':
            dev_f1 = tester.test_classifier(dataset_dev)
            f1 = tester.test_classifier(dataset_test)
